# -*- coding: utf-8 -*-
"""
Created on Wed Apr 06 07:58:24 2016
...
def new_task():
    ...
    if not name or not date or not priority:
        ...
        g.db.execute('insert into tasks (name, due_sate, priority, status)\
        values(?,?,?,1)',[
        request.form['name'],
...
def complete(task_id):
    g.db=connect_db()
    g.db.execute('update wasks set status = 0 where task_id=' + str(task_id))
    g.db.commit()          
    g.db.close()
